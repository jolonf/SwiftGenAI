...
public extension GenAI {
    ...
    func generateContent(model: String, content: String, config: GenerateContentConfig? = nil) async throws -> GenerateContentResponse {
        return try await generateContent(model: model, contents: [Content(parts: [.text(content)])], config: config)
    }

    func generateContent(model: String, content: Content, config: GenerateContentConfig? = nil) async throws -> GenerateContentResponse {
        return try await generateContent(model: model, contents: [content], config: config)
    }

    func generateContent(model: String, part: Part, config: GenerateContentConfig? = nil) async throws -> GenerateContentResponse {
        return try await generateContent(model: model, contents: [Content(parts: [part])], config: config)
    }

    func generateContent(model: String, parts: [Part], config: GenerateContentConfig? = nil) async throws -> GenerateContentResponse {
